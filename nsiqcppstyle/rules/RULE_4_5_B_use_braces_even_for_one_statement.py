...
def RunRule(lexer, contextStack):
    ...
    if t.type in ("IF", "WHILE", "FOR"):
        ...
        if nt is not None and not nt.type in ("SEMI", "LBRACE"):
            nsiqcppstyle_reporter.Error(
                t, __name__, "use brace for even on statement in if/while/for clause")
    elif t.type == "ELSE":
        nt = lexer.GetNextTokenSkipWhiteSpaceAndCommentAndPreprocess()
        if nt is not None and not nt.type in ("IF", "LBRACE"):
            nsiqcppstyle_reporter.Error(
                t, __name__, "use brace for even on statement in else clause")
